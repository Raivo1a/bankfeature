...
@pytest.mark.parametrize(
    "card_number", ["2345346453456755475", "124", "abc"]  # слишком длинный номер  # слишком короткий номер
)  # содержит буквы
def test_get_mask_invalid_card_number(card_number: str) -> None:
...
def test_get_mask_account(card_account: str, expected: str) -> None:
...
def test_get_mask_invalid_account(card_account: str) -> None:
    with pytest.raises(ValueError) as exc_info:
